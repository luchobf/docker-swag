...
auto_envsubst() {
  echo "Substituting env vars in templates..."
  local template_dir="${NGINX_ENVSUBST_TEMPLATE_DIR:-/etc/nginx/templates}"
  local suffix="${NGINX_ENVSUBST_TEMPLATE_SUFFIX:-.template}"
  local output_dir="${NGINX_ENVSUBST_OUTPUT_DIR:-/config/nginx/site-confs}"
  local stream_suffix="${NGINX_ENVSUBST_STREAM_TEMPLATE_SUFFIX:-.stream-template}"
  local stream_output_dir="${NGINX_ENVSUBST_STREAM_OUTPUT_DIR:-/config/nginx/proxy-confs}"
  ...
  echo "Template directory found, continuing with defined_envs: ${defined_envs}"
  if [ ! -w "$output_dir" ]; then
    entrypoint_log "$ME: ERROR: $template_dir exists, but $output_dir is not writable"
    return 0
  fi
  echo "Output directory is writable, continuing..."
  find "$template_dir" -follow -type f -name "*$suffix" -print | while read -r template; do
    ...
    echo "Creating subdirectory: ${output_dir}/${subdir}"
  ...
  if test -n "$(find "$template_dir" -name "*$stream_suffix" -print -quit)"; then
    ...
    find "$template_dir" -follow -type f -name "*$stream_suffix" -print | while read -r template; do
      ...
      entrypoint_log "$ME: Running envsubst on stream template $template to $output_path"
      envsubst "$defined_envs" < "$template" > "$output_path"
    done
  fi
}
